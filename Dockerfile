...
RUN mkdir -p /s_nuxt_2nd/src && mkdir -p /ext/node_modules
...
RUN chown -R user-in-container:user-in-container /s_nuxt_2nd /ext && chmod +x /entry.sh

USER user-in-container

RUN cd /s_nuxt_2nd && tar xjof node_modules.tar.bz2

WORKDIR /s_nuxt_2nd/src
